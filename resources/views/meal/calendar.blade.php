...
<nav id="calendar" class="shadow-sm">
    <div class="month">
        <div class="prev">
            <a aria-label="Bestellübersicht für {{ __('calendar.' . \Carbon::parse($requestedDate)->subMonthNoOverflow()->format('F')) }} {{ $requestedDate->format('Y') }}" href="<?= route(
                'meals.index',
                [
        ...
        <div class="current-month">
            {{ __('calendar.' . $requestedDate->format('F')) }}&nbsp;
            {{ $requestedDate->format('Y') }}
        </div>
        <div class="next">
            <a aria-label="Bestellübersicht für {{ __('calendar.' . \Carbon::parse($requestedDate)->addMonthNoOverflow()->format('F')) }} {{ $requestedDate->format('Y') }}" href="<?= route(
                'meals.index',
                [
                    'date' => \Carbon::parse($requestedDate)
                        ->addMonthNoOverflow()
    ...
    <div class="weekdays" aria-hidden="true" role="presentation">
    ...
    @for($i = 1; $i <= $daysInMonth; $i++)
        @if($date->day == 1 or $date->isMonday())
            <div class="week">
                <div class="day week-of-year" aria-hidden="true">{{ $date->weekOfYear }}</div>
                @endif
                ...
                <div class="day {{ $date->isSameDay($requestedDate) ? ' active' : '' }} {{ $orders->userOrdersForDate($date)->isNotEmpty() ? ' has-orders' : '' }}">
                    <div class="inner">
                        @if ( ($meals->forDate($date)->isNotEmpty()))
                            <a aria-label="Essen bestellen fur {{$date->format(trans('futtertrog.date_format'))}}" href="<?= route('meals.index', ['date' => $date->toDateString()]) ?>"
                            ...
                            @if($orders->userOrdersForDate($date)->isNotEmpty())
                                ...
                                <div class="ordered-meals-tooltip">
                                    ...
                                    @if(count($orders->userOrdersForDate($date)) > 1)
                                        <ol class="text-left my-0 list-unstyled">
                                            @foreach($orders->userOrdersForDate($date) as $order)
                                                <li class="mt-2">{{$order->quantity}} &times; {{ $order->meal->title }}</li>
                                            @endforeach
                                        </ol>

                                    @else
                                        @php
                                            $order = $orders->userOrdersForDate($date)->first();
                                        @endphp
                                        <div class="mt-2">{{$order->quantity}} &times; {{ $order->meal->title }}</div>
                                    @endif
                            ...
                            @else
                                ...
                                <div class="available-meals-tooltip"  class="shadow-sm">
                                    <div class="available-meals-tooltip-title">
                                        {!! __('There are :count order possibilities for :date',['count' => $meals->forDate($date)->count(), 'date' => $date->format(trans('futtertrog.date_format'))]) !!}
                                    </div>
                                </div>
                            @endif
                        @else
                            <span aria-hidden="true" class="a">{{ $date->day }}</span>
                        @endif
                    </div>
                </div>
                @php
